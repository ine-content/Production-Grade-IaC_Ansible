...
# maps device_id -> {"fail_count": N, "fail_status": <int>, "lie": bool}.
# Two unrelated lessons share this same file:
#
#   fail_count / fail_status (TODO 08) - for the first fail_count PATCH
#   attempts to a given device, this mock responds with fail_status
#   instead of a real push; every attempt after that behaves normally.
#   Setting fail_count to a very large number simulates a permanent
#   failure that never recovers - exactly what a real 401/403/422 looks
#   like from a pipeline's point of view.
#
#   lie (TODO 09) - this device accepts every PATCH with a completely
#   normal 204, but never actually updates its stored state. A push
#   result that trusts its own success status code would never notice;
#   only a real GET afterward would.
#
# A device with no entry here always behaves normally - this file only
# exists at all when a lesson specifically needs to simulate a flaky,
# broken, or lying device. Both graders start this fleet FIRST and only
# write this file afterward, against the same already-running process -
# so it must be read fresh on every PATCH (see do_PATCH below), never
# cached once at server startup, or a fault written after the server is
# already up would silently never take effect.
...
# happened since exactly that entry took effect. A grader's clean push
...
def record_patch_attempt(device_id, fault):
    ...
    failure, which would corrupt TODO 09's own lying-device check.
...
def make_handler(device_id, expected_path, expected_credentials):
    ...
    class DeviceHandler(BaseHTTPRequestHandler):
        ...
        def do_PATCH(self):
            ...
            # TODO 09's specific trap: a device configured to "lie" accepts
            # the PATCH and returns a completely normal 204 - exactly what
            # a real, successful push looks like - but never actually
            # updates its stored state. A pipeline that trusts the PATCH's
            # own status code as proof the device changed would never
            # catch this; only an independent GET-and-compare afterward
            # would ever reveal that nothing actually moved server-side.
            if not (fault or {}).get("lie"):
                with STATE_LOCK:
