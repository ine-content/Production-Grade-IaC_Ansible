...
def vlan_marker(platform, vlan_id):
    """The substring that would reveal a given VLAN ID's presence in a
    device's own rendered output - platform-specific, mirroring each
    template's own rendering rule exactly (see templates/*.j2). Nexus 9K
    is a switch and renders a VLAN database entry ("vlan <id>"). CAT8K is
    a router with no VLAN database of its own - it renders an OSPF
    network statement for that VLAN's derived subnet instead, so its
    marker is that subnet ("10.<id // 10>.0.0"). Used to build
    disabled_vlan_markers below, for TODO 06's independent policy
    re-check - a device's rendered text should never contain the marker
    for a VLAN its own environment disables."""
    if platform == "cat8k":
        return f"10.{vlan_id // 10}.0.0"
    ...
    for device_file in sorted(DEVICES_DIR.glob("*.json")):
        for device in load_json(device_file):
            ...
            inventory["_meta"]["hostvars"][host] = {
                ...
                # TODO 06's independent policy re-check needs to know which
                # VLANs this device's own environment disables, in the
                # exact substring form that would reveal them in this
                # device's own rendered text - computed here (once, from
                # site["vlan_ids"] and the environment's disabled_roles),
                # not re-derived from resolved_vlans, since resolved_vlans
                # has already had disabled VLANs filtered out and so could
                # never reveal a leak by itself.
                "disabled_vlan_markers": [
                    vlan_marker(device["platform"], site["vlan_ids"][role])
                    for role in environment_policy.get("disabled_roles", [])
                    if role in site["vlan_ids"]
                ],
            }
