...
TODO 10 is checked by deleting any existing logs/audit.jsonl, running one
final clean playbook pass with nothing faked, then reading that file back
directly - file- and schema-based, never text-matched against stdout.
Every one of the 9 devices must have exactly one well-formed JSON line,
with device_id/platform/site_id/environment matching that device and
pushed/push_status/verified reflecting its real outcome: the 2 staging
devices should show pushed: true, push_status: 204, verified: true; the 7
production devices should show pushed: false, push_status: null,
verified: false. A record with a string "true"/"204" instead of a real
JSON boolean/int/null fails this just as much as a missing or wrong line
does - this TODO exists to make sure the values stay real JSON types, not
just correct-looking text.
"""
...
AUDIT_LOG_FILE = ROOT / "logs" / "audit.jsonl"
...
TODO_NAMES = {
    ...
    10: "Emit a Structured Audit Log",
}
...
SUCCESS_MESSAGES = {
    ...
    10: "Every device appended a correct, permanent audit record to logs/audit.jsonl - pushed, held, and verified outcomes are now on the record, not just in a terminal that already closed.",
}
...
FAIL_MESSAGES = {
    ...
    10: "The pipeline cannot complete because a correct, permanent audit record was not appended to logs/audit.jsonl for every device.",
}
...
HINTS = {
    ...
    6: "Write two assert tasks. First: lookup('file', ...output.../<device_id>.cfg) equals lookup('file', ...golden.../<device_id>.cfg). Second: disabled_vlan_markers (already computed for you in inventory/devices.py) | select('in', <that same rendered text>) | list | length == 0 - the Jinja 'in' test does substring containment when the right side is a string, so this finds any disabled VLAN marker that leaked into the rendered text, independently of golden - register: policy_check_result on this second task only (if it runs at all, the first one already passed).",
    ...
    10: "Build one Jinja dict literal with all 7 keys (device_id, platform, site_id, environment, pushed, push_status, verified) and pipe the WHOLE dict through | to_json inside a single {{ ... }} expression, in one pass. Templating a vars: dict field-by-field first (like earlier TODOs did for render_context) flattens every value to plain text before to_json ever sees it - true/204/null would come out as the strings \"True\"/\"204\"/\"None\" instead of real JSON types. Then use ansible.builtin.shell (not lineinfile/copy/blockinfile - those rewrite the whole file and can silently lose another host's concurrent write) to append that one line with a shell redirect: printf '%s\\n' {{ audit_line | quote }} >> {{ playbook_dir }}/logs/audit.jsonl.",
}
...
- name: assert the device's live state matches what was pushed
  ...
  ansible.builtin.assert:
    ...
    10: '''- name: append this device's audit record to the shared audit log
  vars:
    audit_line: >-
      {{ {'device_id': device_id,
          'platform': platform,
...
EXPECTED = {
    ...
    10: "After a clean run, logs/audit.jsonl should contain exactly one JSON line per device (9 total), with device_id/platform/site_id/environment matching that device, and pushed/push_status/verified reflecting its real outcome: true/204/true for the 2 staging devices, false/null/false for the 7 production devices - as real JSON types, not text that merely looks right.",
}
...
PROBLEMS = {
    ...
    10: "logs/audit.jsonl is missing, has the wrong number of lines, contains invalid JSON, has a duplicate device_id, or one or more fields (device_id/platform/site_id/environment/pushed/push_status/verified) don't match that device's real outcome or aren't real JSON types.",
}
...
def check_todo_10():
    """Deletes any existing logs/audit.jsonl (stale lines from an earlier
    run inside this same grading session - fault-injection runs from
    check_todo_8/check_todo_9 included - would otherwise hide a bug, or
    make an old duplicate device_id look like a fresh failure), runs one
    final clean playbook pass with nothing faked, then reads the file
    back directly.

    File- and schema-based, never text-matched against stdout: every one
    of the 9 devices must have exactly one well-formed JSON line, with
    device_id/platform/site_id/environment matching that device, and
    pushed/push_status/verified reflecting its real outcome - true/204/
    true for the 2 staging devices, false/None/false for the 7
    production devices. json.loads() is what actually enforces "real
    JSON types" here: a solution that wrote the text "true" instead of
    JSON's bare true would still round-trip through json.loads() as the
    four-character string "true", which fails record.get("pushed") ==
    True just as loudly as a missing field would."""
    expected_devices = load_expected_devices()
    diagnostics = {}
    ...
    if not AUDIT_LOG_FILE.exists():
        return False, {"logs/audit.jsonl": "does not exist - no device appended an audit record."}
    ...
    for line in lines:
        try:
            ...
            diagnostics["logs/audit.jsonl"] = f"contains a line that is not valid JSON: {line!r}"
            continue
        device_id = record.get("device_id")
        if device_id in records_by_device:
            diagnostics[device_id] = (
                f"appears more than once in logs/audit.jsonl - expected exactly one line per device."
            )
        records_by_device[device_id] = record

    for device_id, expected in expected_devices.items():
        if device_id in diagnostics:
            continue
        record = records_by_device.get(device_id)
        if record is None:
            diagnostics[device_id] = f"has no line at all in logs/audit.jsonl."
            continue

        expected_pushed = device_id in STAGING_DEVICES
        expected_fields = {
            "device_id": device_id,
            "platform": expected["platform"],
            "site_id": expected["site_id"],
            "environment": expected["environment_name"],
            "pushed": expected_pushed,
            "push_status": 204 if expected_pushed else None,
            "verified": expected_pushed,
        }
        mismatches = [
            f"{key} was {record.get(key)!r} (a {type(record.get(key)).__name__}), expected {value!r}"
            for key, value in expected_fields.items()
            if record.get(key) != value
        ]
        if mismatches:
            diagnostics[device_id] = "; ".join(mismatches)
...
def compute_statuses():
    ...
    # fleet is torn down.
    # TODO 9 shares that same fleet session - it needs a real push to
    # verify against - so there's no reason to tear the fleet down and
    # start a second one until TODO 10 is also done with it. TODO 10
    # itself doesn't need the mock fleet for its own logic (it only reads
    # push_result/verify_result, already set by earlier tasks, and
    # writes a local file) - but it does need a real playbook run to
    # populate those variables in the first place, so it stays inside
    # this same session too, right after TODO 9 confirms correct.
    todo_8_status = False
    todo_8_diagnostics = {}
    todo_9_status = False
    todo_9_diagnostics = {}
    todo_10_status = False
    todo_10_diagnostics = {}
    if statuses[7]:
    ...
    context = {
        ...
        "todo_10_diagnostics": todo_10_diagnostics,
        "expected_devices": load_expected_devices(),
    }
...
def print_todo_details(number, statuses, context):
    if number == 1:
        ...
        console.print("[10] Emitting a structured audit log...")
        if statuses[number]:
            expected_devices = context["expected_devices"]
            for host in sorted(expected_devices.keys()):
                if host in STAGING_DEVICES:
                    console.print(f"  {host} (staging) -> pushed: true, push_status: 204, verified: true")
                else:
                    console.print(f"  {host} (prod) -> pushed: false, push_status: null, verified: false")
            console.print("logs/audit.jsonl has exactly one correct line per device.")
        else:
            diagnostics = context.get("todo_10_diagnostics") or {}
            for host in sorted(diagnostics.keys()):
                console.print(f"  {host} -> {diagnostics[host]}")
