...
TODO 01, and stops at the first incomplete one - even though TODO 01 is
already solved for you in this folder, it re-checks it live every run,
so you always see real, current proof that nothing earlier broke, not a
cached checkmark.

TODO 02 is checked by running the real playbook -
    ansible-playbook -i inventory/devices.py site.yml
against the real dynamic inventory, once, and reading its output.
"""
...
def check_todo_2(result):
    ...
    if "unreachable=0" not in text:
        return False
    if text.count("failed=0") < len(expected_devices):
        return False
    ...
    if number == 1:
        ...
        if statuses[number]:
            ...
            for host in sorted(expected_devices.keys()):
                ...
                console.print(f"  {host} ({d['environment_name']}, {d['platform']}) resolved_vlans: {d['vlan_ids']}")
